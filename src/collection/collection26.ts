...
export function groupProductsByPrice(products: Product[]): GroupedProducts {
	...
	{
	cheap: products.filter((product) => product. price < 1000),
	medium: products.filter((product) => product. price >= 1000 && product.price <= 5000),
	expensive: products.filter((product) => product. price > 5000)
	}
